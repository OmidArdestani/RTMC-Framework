...
/* Task function to perform computations on core 1 */
static void prvCore1ComputingTask(void *pvParameters);
...
static void prvCore1Entry() {
    ...
    /* Create the computing task for core 1 */
    xTaskCreate(prvCore1ComputingTask,       // Task function
                "Core1ComputingTask",        // Task name
...
static void prvCore1ComputingTask(void *pvParameters) {
    ...
    for (;;) {
        counter++;
        printf("Core 1: Computing task running, counter = %u\n", counter);
...
static void vExampleTimerCallback( TimerHandle_t xTimer )
{
    ...
    // if(ulCountOfTimerCallbackExecutions%2 == 0) {
    //     gpio_put(LED_RED_PIN, 1); // Turn on LED on GPIO 14
    // }
    // else if(ulCountOfTimerCallbackExecutions%5 == 0){
    //     gpio_put(LED_GREEN_PIN, 1); // Turn on LED on GPIO 14
    // }
    // else {
    //     gpio_put(LED_YELLOW_PIN, 1); // Turn off LED on GPIO 14
    // }

    // gpio_put(PICO_DEFAULT_LED_PIN, 1); // Turn off LED on GPIO 14
}
/*-----------------------------------------------------------*/
...
static void prvADCReadTask(void *pvParameters)
{
    while (true)
    {
        adc_select_input(1); // Select ADC1 (GPIO 27)
        uint16_t adc_value = adc_read(); // Read ADC value (12-bit resolution)
        float voltage = (adc_value * 3.3f) / 4095.0f; // Convert to voltage (assuming 3.3V reference)

        printf("ADC1 Value: %u, Voltage: %.2f V\n", adc_value, voltage);

        if(voltage>2.5f) {
            gpio_put(LED_RED_PIN, 0); // Turn on red LED
            gpio_put(LED_GREEN_PIN, 1); // Turn off green LED
            gpio_put(LED_YELLOW_PIN, 0); // Turn off yellow LED
        } else {
            gpio_put(LED_YELLOW_PIN, 0); // Turn on yellow LED
            gpio_put(LED_RED_PIN, 1); // Turn off red LED
...
static void prvSetupHardware( void )
{
    ...
    gpio_put(PICO_DEFAULT_LED_PIN, 1); // Turn off LED on GPIO 14 initially

    
    /* Initialize GPIO pins 14, 15, and 16 for LEDs */
    gpio_init(LED_RED_PIN);
    gpio_set_dir(LED_RED_PIN, GPIO_OUT);
    gpio_put(LED_RED_PIN, 0); // Turn off LED on GPIO 14 initially

    gpio_init(LED_GREEN_PIN);
    gpio_set_dir(LED_GREEN_PIN, GPIO_OUT);
    gpio_put(LED_GREEN_PIN, 0); // Turn off LED on GPIO 15 initially

    gpio_init(LED_YELLOW_PIN);
    gpio_set_dir(LED_YELLOW_PIN, GPIO_OUT);
    gpio_put(LED_YELLOW_PIN, 0); // Turn off LED on GPIO 16 initially
